...
function NavItem({ href, icon, label, badge, exact, excludes }: NavItemProps) {
  ...
  return (
    ...
    >
      <span className={cn('w-4 h-4 flex items-center justify-center flex-shrink-0', active ? 'opacity-100' : 'opacity-60')}>
        {icon}
      </span>
      {label}
      {badge && (
        <span className="ml-auto bg-primary text-white font-display text-[10px] font-bold px-1.5 py-px rounded-full">
          {badge}
        </span>
      )}
    </Link>
...
export function Sidebar({
  ...
  return (
    <aside className="w-sidebar min-w-sidebar bg-white border-r border-border flex flex-col sticky top-0 h-screen">
      ...
      <div className="px-2.5 py-3.5 border-t border-border">
        <div className="flex items-center gap-2.5 px-2.5 py-2 rounded hover:bg-sidebar-hover transition-colors">
          <div className="w-7 h-7 rounded-full bg-primary-lt border border-primary-border flex items-center justify-center font-display text-[10px] font-bold text-primary flex-shrink-0">
            {initials}
          </div>
