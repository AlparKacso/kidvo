...
export function BottomNav({ isProvider = false, providerPendingTrials = 0, providerWaitingCount = 0, userEmail = '', hasEvents = false }: Props) {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-border flex items-stretch safe-area-bottom">
      ...
      {!isProvider && (
        <NavItem href="/kids/calendar" icon={<IconCalendar />} label={t('calendar')} />
      )}
      {isProvider && (
